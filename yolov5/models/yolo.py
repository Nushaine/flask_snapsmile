...
class Model(nn.Module):
    ...
    def __init__(self, model_cfg='yolov5s.yaml', ch=3, nc=None):
        ...
        self.model, self.save = parse_model(
            self.md, ch=[ch])  # model, savelist, ch_out
    ...
    def _print_biases(self):
        ...
        for f in sorted([x % m.i for x in m.f]):  #  from
            b = self.model[f].bias.detach().view(
                m.na, -1).T  # conv.bias(255) to (3,85)
            print(('%g Conv2d.bias:' + '%10.3g' * 6) %
                  (f, *b[:5].mean(1).tolist(), b[5:].mean()))
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            if type(m) is Conv:
                m.conv = torch_utils.fuse_conv_and_bn(
                    m.conv, m.bn)  # update conv
...
def parse_model(md, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(md['backbone'] + md['head']):
        ...
        if m in [nn.Conv2d, Conv, Bottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, C3]:
            ...
            f = f or list(reversed([(-1 if j == i else j - 1)
                                    for j, x in enumerate(ch) if x == no]))
        else:
            c2 = ch[f]
